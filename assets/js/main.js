...
(function () {
  ...
  if (galleryTriggers.length) {
    ...
    // clic sur les bords de l'écran = navigation ; clic sur le fond = fermeture
    lightbox.addEventListener("click", function (e) {
      if (e.target === lbClose || lbClose.contains(e.target)) return; // le bouton fermer gère déjà son propre clic

      var zone = window.innerWidth / 6;
      if (e.clientX < zone) {
        nextImage(); // bande gauche (1/6 de l'écran) → image suivante
        return;
      }
      if (e.clientX > window.innerWidth - zone) {
        prevImage(); // bande droite (1/6 de l'écran) → image précédente
        return;
      }
      if (e.target === lightbox) closeLightbox(); // clic au centre, hors image → fermeture
    });
